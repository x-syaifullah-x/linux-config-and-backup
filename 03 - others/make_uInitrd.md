...
for _dir in etc home opt root srv usr var; do
...
mount -t ramfs -o rw,noatime ramfs $rootmnt/boot || exit $?
...
### PACKAGES
```sh
apt install --no-install-suggests --no-install-recommends u-boot-tools lz4
```
### MAKE INITRD
```sh
update-initramfs -v -d -c -k 6.6.69-current-meson64
```
### 
```sh
mkimage -A arm64 -O linux -T ramdisk -C gzip -a 0x80000000 -e 0x80000000 -n "uInitrd" -d initrd.img-6.6.69-current-meson64 uInitrd
```